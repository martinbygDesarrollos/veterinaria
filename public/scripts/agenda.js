		...
		if ( response.result == 2 ){
			if ( response.listResult.length > 0){
				...
				for (var i = 0; i < response.listResult.length; i++) {
					...
					row = '<tr id="'+ response.listResult[i].idAgenda +'" onchange="saveEventInCalendar(this)"><td><input class="form-control text-center shadow-sm" type="time" name="" value="'+ hora +'"></td><td class="w-50"><input class="form-control text-center shadow-sm" type="text" name="" value="'+ response.listResult[i].descripcion +'" placeholder="Motivo" ></td><td><input class="form-control text-center shadow-sm" type="text" name="" value="" onkeyup="searchClientByName(this.value)" list="dataListClientsCalendar" placeholder="Cliente"><datalist id="dataListClientsCalendar"></datalist></td><td><input class="form-control text-center shadow-sm" type="text" name="" value="" onkeyup="searchPetClientByName(this.value, this.parentElement.parentElement)" list="dataListPetCalendar" placeholder="Mascota"><datalist id="dataListPetCalendar"></datalist></td></td></tr>';
